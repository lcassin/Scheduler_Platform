...
";
    ...
    /// diagram types that lay out based on container width (ZenUML, Gantt,
    /// architecture) pick up the new viewport size.
    /// </summary>
    ...
    /// <summary>
    /// Re-render the preview when the window is resized (e.g. dragging edges)
    /// so responsive diagram types adapt to the new container width, matching
    /// the behaviour of mermaid.live.  Debounced via _renderTimer.
    /// </summary>
                        ...
                        requestAnimationFrame(() => {{
                            ...
                            if (svgWidth > 0 && svgHeight > 0) {{
                                ...
                                if (usesContainerWidth) {{
                                    ...
                                    // Fixed-width types: remove maxWidth cap and force to content size.
...
<body>
    ...
    private async void VisualEditorBridge_EditorReady(object? sender, EventArgs e)
    {
        ...
        if (_visualEditorMode != VisualEditorMode.Text)
        {
            ...
            else if (_currentZenUMLModel != null)
            {
                await _visualEditorBridge.UpdateZenUMLModelAsync(_currentZenUMLModel);
            }
    ...
    /// <summary>
    /// Called when the visual editor modifies the ZenUMLModel.
    /// Serializes the model back to text and updates the code editor + preview.
    /// </summary>
    private async void VisualEditorBridge_ZenUMLModelChanged(object? sender, ZenUMLModelChangedEventArgs e)
    {
        if (_isVisualEditorUpdating) return;
        ...
        try
        {
            var text = MermaidSerializer.SerializeZenUML(e.Model);
            ...
            if (_visualEditorMode == VisualEditorMode.Visual || _visualEditorMode == VisualEditorMode.Split)
            {
                ...
                UpdateTitle();
                RenderPreview();
                await _visualEditorBridge.RefreshZenUMLAsync();
            }
        }
        finally
        {
    ...
    private void SwitchToTextMode()
    {
        if (_visualEditorMode != VisualEditorMode.Text && _visualEditorBridge != null)
        {
            ...
            try
            {
                ...
                {
                    text = MermaidSerializer.SerializeZenUML(_currentZenUMLModel);
                }
                else if (_currentXYChartModel != null)
                {
    ...
    private async Task ParseAndSendToVisualEditor()
    {
        ...
        try
        {
            ...
            else if (IsZenUMLDiagram(text))
            {
                var parsed = MermaidParser.ParseZenUML(text);
                if (parsed != null)
                {
                    ClearAllModels();
                    _currentZenUMLModel = parsed;
                    await _visualEditorBridge.UpdateZenUMLModelAsync(_currentZenUMLModel);
                }
            }
    ...
    /// <summary>
    /// Detects whether the given Mermaid text is a ZenUML diagram.
    /// </summary>
    private static bool IsZenUMLDiagram(string text)
    {
        var line = GetFirstMeaningfulMermaidLine(text);
        return line != null && line.Equals("zenuml", StringComparison.OrdinalIgnoreCase);
    }
