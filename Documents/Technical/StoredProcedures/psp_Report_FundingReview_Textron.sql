...
GO
--exec psp_Report_FundingReview_Textron @FundingDate = '12/25/2025'
...
-- 2026/02/27				Devin AI	Fix blank BU on Account rows: corrected reversed IS NULL conditions
--										(r1 -> r2) on CostCenterId/GLLinkId fallback updates, and replaced
--										single Account row insertion with proportional CostCenter allocation
--										to correctly handle multi-BU invoices (e.g. Verizon Wireless, AT&T Mobility).
...
--***************************************************************
	...
	if object_id('tempdb..[#tmpCCProportion]','u') is not null 
			drop table #tmpCCProportion
   ...
   GROUP BY bi.BillingMonth,bi.InvoiceNumber, totalOrig, tmp.BillerInvoiceID, CF.CurrencyCode
	...
	--Insert records for account level charges, proportionally allocated across CostCenters
	--This handles multi-BU invoices (e.g. Verizon Wireless, AT&T Mobility) by splitting
	--invoice-level taxes/misc proportionally based on each CostCenter's share of total charges.

	-- Step 1: Build CostCenter proportions from ChargesumAllocated
	SELECT 
		al.BillerInvoiceId,
		al.CostCenterId,
		SUM(al.ChargeAmount) AS CCCharge,
		ROW_NUMBER() OVER (PARTITION BY al.BillerInvoiceId ORDER BY ABS(SUM(al.ChargeAmount)) DESC) as rn
	INTO #tmpCCProportion
	FROM tbl_All_ChargesumAllocated al WITH (NOLOCK)
	WHERE al.BillerInvoiceId IN (SELECT BillerInvoiceID FROM #tmpInvDiff)
		AND al.Client_Number = @ClientNo
	GROUP BY al.BillerInvoiceId, al.CostCenterId
	HAVING SUM(al.ChargeAmount) <> 0

	ALTER TABLE #tmpCCProportion ADD TotalCharge money NULL, ProportionalAmount money NULL

	-- Calculate total charge per invoice across all CostCenters
	UPDATE p SET TotalCharge = t.TotalCharge
	FROM #tmpCCProportion p
	INNER JOIN (
		SELECT BillerInvoiceId, SUM(CCCharge) as TotalCharge 
		FROM #tmpCCProportion 
		GROUP BY BillerInvoiceId
	) t ON t.BillerInvoiceId = p.BillerInvoiceId

	-- Step 2: Calculate proportional amounts for non-primary CostCenters (rn > 1)
	UPDATE p SET ProportionalAmount = 
		CASE WHEN p.TotalCharge = 0 THEN 0
			 ELSE ROUND(d.InvoiceDiff * p.CCCharge / p.TotalCharge, 2)
		END
	FROM #tmpCCProportion p
	INNER JOIN #tmpInvDiff d ON d.BillerInvoiceID = p.BillerInvoiceId
	WHERE p.rn > 1

	-- Step 3: For primary CostCenter (rn = 1, largest share), assign the remainder
	-- to ensure total of all Account rows equals InvoiceDiff exactly (no rounding drift)
	UPDATE p SET ProportionalAmount = 
		d.InvoiceDiff - ISNULL(others.OtherTotal, 0)
	FROM #tmpCCProportion p
	INNER JOIN #tmpInvDiff d ON d.BillerInvoiceID = p.BillerInvoiceId
	LEFT JOIN (
		SELECT BillerInvoiceId, SUM(ProportionalAmount) as OtherTotal 
		FROM #tmpCCProportion 
		WHERE rn > 1 
		GROUP BY BillerInvoiceId
	) others ON others.BillerInvoiceId = p.BillerInvoiceId
	WHERE p.rn = 1

	-- Step 4: Insert Account rows with proportional CostCenter allocation
	INSERT INTO ##Final_TextronFunding_Rpt(BillerInvoiceId,Business_Unit,Company,[GL Cost Account],Currency,MasterAccountNo,Account_Number
											...
											,Batch_Name,First_Name,Last_Name,EmployeeID,City,State,Address_Line)
	Select d.BillerInvoiceID,null,null,null,d.CurrencyCode,null,null,null,null,null,d.InvoiceNumber,null,null,d.BillingMonth,
			p.CostCenterId,null,null,null,p.ProportionalAmount,
			'Account','Account',null,null,null,null,null,null,null,null
	from #tmpInvDiff d WITH (NOLOCK)
	INNER JOIN #tmpCCProportion p ON p.BillerInvoiceId = d.BillerInvoiceID

	-- Fallback: Insert Account rows for invoices that had no ChargesumAllocated data
	INSERT INTO ##Final_TextronFunding_Rpt(BillerInvoiceId,Business_Unit,Company,[GL Cost Account],Currency,MasterAccountNo,Account_Number
											,Sub_Billing,GLLinkId,vendor,InvoiceNumber,AccountDesc,InvoiceDate,BillingMonth,CostCenterId
											,CostCenter,EntityName,Invoice_Total,Allocated_Amount,Identifier,Inventory_Type,Circuit_Desc
											,Batch_Name,First_Name,Last_Name,EmployeeID,City,State,Address_Line)
	Select BillerInvoiceID,null,null,null,CurrencyCode,null,null,null,null,null,InvoiceNumber,null,null,BillingMonth,null,null,null,null,InvoiceDiff
			,'Account','Account',null,null,null,null,null,null,null,null
	from #tmpInvDiff d WITH (NOLOCK)
	WHERE NOT EXISTS (SELECT 1 FROM #tmpCCProportion p WHERE p.BillerInvoiceId = d.BillerInvoiceID)
	...
	inner join ##Final_TextronFunding_Rpt r2 WITH (NOLOCK)
		ON r2.BillerInvoiceId = r1.BillerInvoiceId
		AND (r1.Inventory_Type  <> 'Account' OR r1.Inventory_Type IS NULL)
	Where r2.Inventory_Type = 'Account'  AND r2.CostCenterId IS NULL
	...
	inner join ##Final_TextronFunding_Rpt r2 WITH (NOLOCK)
		ON r2.BillerInvoiceId = r1.BillerInvoiceId
		AND (r1.Inventory_Type  <> 'Account' OR r1.Inventory_Type IS NULL)
	Where r2.Inventory_Type = 'Account' AND r2.GLLinkId IS NULL
    ...
    /* UPDATE ##Final_TextronFunding_Rpt SET Identifier=a.Identifier,
                                          Inventory_Type=a.Inventory_Type                                                       
    FROM ##Final_TextronFunding_Rpt tmp 
        INNER JOIN (SELECT CASE WHEN a.Identifier='Account' THEN b.Identifier 
                      WHEN b.Identifier='Account' THEN a.Identifier
                      ELSE a.Identifier END AS Identifier,
                      CASE WHEN a.Inventory_Type='Account' THEN b.Inventory_Type
                      WHEN b.Inventory_Type='Account' THEN a.Inventory_Type
                      ELSE a.Inventory_Type END AS Inventory_Type,
                      a.GLLinkID                                                         
        FROM ##Final_TextronFunding_Rpt a
            INNER JOIN ##Final_TextronFunding_Rpt b
                ON a.GLLinkID = b.GLLinkId
                AND a.Identifier <> b.Identifier) a
            ON tmp.GLLinkID=a.GLLinkID */
